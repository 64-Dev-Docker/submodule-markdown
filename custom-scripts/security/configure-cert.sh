...
git config --global user.name $(gpg --list-secret-keys |  perl -n -e '/\[ultimate\]\s*([a-zA-Z]*)\s*/ && print $1')
git config --global user.email $(gpg --list-secret-keys |  perl -n -e '/\[ultimate\].*<(.*)>/ && print $1')
